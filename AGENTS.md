...
1.  **Script Priority**: ALWAYS use the scripts in `scripts/` (e.g., `scripts/local/start.sh`, `scripts/local/test.sh`) instead of raw commands like `docker-compose up` or `pytest`. These scripts handle environment setup and cleanup correctly.
...
3.  **Verification Before Review**: Before requesting a review or completing a task, you MUST:
    ...
    - **Backend**: Ensure all tests pass via `./scripts/local/test.sh`.
    - **Linter Ignores**: Using `eslint-disable` or equivalent linter ignores is STRICTLY DISCOURAGED. You must find the root cause and fix it properly. Only use ignores as a last resort for third-party code inconsistencies, and explain why in your PR/comment.
4.  **Clean Code**:
...
- **Test Plan**: Explicitly state how you verified the change (e.g., "Ran `scripts/local/test.sh` and verified unit tests pass", or "Manually checked localhost:3000/login").
