...
class TestSecurityAuditLogging:
    ...
    def test_risk_level_determination(
        self, event_type: SecurityEventType, expected_risk: SecurityRiskLevel
    ):
...
class TestSecurityAuditLogger:
    ...
    def test_log_event_with_request(
        self, db_session: Session, test_user: User, mock_request
    ):
    ...
    def test_ip_address_extraction(self, db_session: Session, header_scenario):
        ...
        audit_log = logger.log_event(
            event_type=SecurityEventType.LOGIN_SUCCESS, request=mock_request
        )
    ...
    def test_rate_limit_event_logging(self, db_session: Session, mock_request):
        ...
        audit_log = logger.log_rate_limit_event(
            request=mock_request, limit_type="ip", limit_value=5, current_count=6
        )
    ...
    def test_user_security_events_retrieval(self, db_session: Session, test_user: User):
        ...
        logger.log_event(
            SecurityEventType.LOGIN_SUCCESS, user=test_user, ip_address="192.168.1.100"
        )
        logger.log_event(
            SecurityEventType.LOGIN_FAILED, user=test_user, ip_address="192.168.1.100"
        )
        logger.log_event(
        ...
        medium_risk_events = logger.get_user_security_events(
            test_user.id, risk_levels=[SecurityRiskLevel.MEDIUM]
        )
    ...
    def test_high_risk_events_retrieval(
        self, db_session: Session, test_user: User, mock_request
    ):
        ...
        logger.log_event(
            SecurityEventType.ACCOUNT_LOCKED, user=test_user, ip_address="192.168.1.100"
        )  # HIGH
    ...
    def test_application_logging_integration(
        self, mock_logger, db_session: Session, mock_request
    ):
        ...
        logger.log_suspicious_activity(
            request=mock_request, activity_type="test", description="Critical event"
        )
