...
class TestModelValidation:
    ...
    def test_user_unique_constraints(
        self, db_session: Session, field: str, value1: str, value2: str
    ):
...
class TestModelRelationships:
    ...
    def test_recipe_permission_relationship(
        self, db_session: Session, test_user: User, test_recipe: Recipe
    ):
        """Test recipe permission relationships."""
        # Get existing permission (created by fixture)
        permission = (
            db_session.query(RecipePermission)
            .filter_by(user_id=test_user.id, recipe_id=test_recipe.id)
            .first()
        )
    ...
    def test_ingredient_recipe_relationship(
        self, db_session: Session, test_recipe: Recipe
    ):
    ...
    def test_instruction_recipe_relationship(
        self, db_session: Session, test_recipe: Recipe
    ):
    ...
    def test_cascade_delete_user(self, db_session: Session):
        ...
        permission = RecipePermission(
            user_id=user.id, recipe_id=recipe.id, role=PermissionRole.OWNER
        )
        ...
        remaining_permission = (
            db_session.query(RecipePermission)
            .filter(RecipePermission.user_id == user.id)
            .first()
        )
        ...
        remaining_recipe = (
            db_session.query(Recipe).filter(Recipe.id == recipe.id).first()
        )
    ...
    def test_cascade_delete_recipe(self, db_session: Session, test_user: User):
        ...
        assert (
            db_session.query(RecipePermission)
            .filter(RecipePermission.recipe_id == recipe.id)
            .first()
            is None
        )
        assert (
        ...
        assert (
            db_session.query(Instruction)
            .filter(Instruction.recipe_id == recipe.id)
            .first()
            is None
        )
...
class TestModelConstraints:
    ...
    def test_recipe_permission_unique_constraint(
        self, db_session: Session, test_user: User, test_recipe: Recipe
    ):
        ...
        permission1 = RecipePermission(
            user_id=test_user.id, recipe_id=test_recipe.id, role=PermissionRole.OWNER
        )
        ...
        permission2 = RecipePermission(
            user_id=test_user.id, recipe_id=test_recipe.id, role=PermissionRole.EDITOR
        )
    ...
    @pytest.mark.parametrize("role", [PermissionRole.OWNER, PermissionRole.EDITOR])
    def test_permission_role_enum_values(
        self,
        db_session: Session,
        test_user: User,
        test_recipe: Recipe,
        ...
        permission = RecipePermission(
            user_id=test_user.id, recipe_id=test_recipe.id, role=role
        )
        db_session.add(permission)
        db_session.commit()
...
class TestModelRepresentations:
    ...
    def test_recipe_repr(self, db_session: Session):
        ...
        recipe = Recipe(title="Repr Test Recipe", cooking_time=30)
        db_session.add(recipe)
        db_session.commit()
        db_session.refresh(recipe)

        expected = (
            f"<Recipe(id={recipe.id}, title='Repr Test Recipe', cooking_time=30)>"
        )
        assert repr(recipe) == expected

    def test_recipe_permission_repr(
        self, db_session: Session, test_user: User, test_recipe: Recipe
    ):
