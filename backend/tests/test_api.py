...
class TestUserAPI(APITestBase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                201,
            ),  # API may not validate email format strictly
            (
                {"username": "user", "email": "valid@example.com", "password": "weak"},
                422,
            ),  # Weak password validation may return 422
        ],
    )
    ...
    def test_user_registration_duplicate_constraints(
    ...
    @pytest.mark.parametrize(
        "invalid_token_scenario",
        ["invalid_token", "access_token_as_refresh", "empty_token"],
    )
    def test_token_refresh_failures(
        self, client: TestClient, test_user: User, invalid_token_scenario: str
    ):
    ...
    def test_user_profile_operations(
        self, client: TestClient, test_user: User, test_user2: User, auth_headers: dict
    ):
...
class TestRecipeAPI(APITestBase):
    ...
    def test_recipe_creation_success(
        self,
        client: TestClient,
        db_session: Session,
        auth_headers: dict,
        ...
        recipe = (
            ...
            .filter(Recipe.title == sample_recipe_data["title"])
            .first()
        )
        assert recipe is not None

        permission = (
            db_session.query(RecipePermission)
            .filter(
    ...
    def test_recipe_creation_with_nested_data(
        ...
        recipe_data = {
            ...
            "ingredients": [
                {"name": "Flour", "quantity": 2.0, "unit": "cups", "order_index": 0}
            ],
            "instructions": [
                {
                    "title": "Mix",
    ...
    def test_recipe_creation_unauthorized(
        self, client: TestClient, sample_recipe_data: dict
    ):
    ...
    def test_my_recipes_endpoint(
        self, client: TestClient, test_recipe: Recipe, auth_headers: dict
    ):
    ...
    def test_recipe_update_operations(
        ...
        update_data = {"title": "Updated Recipe Title", "cooking_time": 60}

        # Test successful update by owner
        response = client.put(
            f"/api/v1/recipes/{test_recipe.id}", json=update_data, headers=auth_headers
        )
        data = self.assert_success_response(response)
    ...
    def test_recipe_deletion_operations(
        self,
        client: TestClient,
        db_session: Session,
        test_recipe: Recipe,
        ...
        response = client.delete(
            f"/api/v1/recipes/{recipe_id}", headers=auth_headers_user2
        )
        self.assert_error_response(response, 404, "Recipe not found or access denied")
    ...
    def test_recipe_permissions_management(
        ...
        permission_data = {"username": test_user2.username, "role": "editor"}
    ...
    def test_recipe_error_scenarios(
        self, client: TestClient, auth_headers: dict, endpoint_scenario
    ):
