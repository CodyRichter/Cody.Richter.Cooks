...
class TestDatabaseTransactions:
    ...
    def test_transaction_isolation(self, db_session: Session):
        ...
        session_user = (
            db_session.query(User).filter(User.username == "isolationtest").first()
        )
...
class TestDatabaseRelationships:
    ...
    def test_create_related_entities(self, db_session: Session):
        ...
        # Create recipe
        recipe = Recipe(
            title="Relationship Test Recipe", description="<p>Testing relationships</p>"
        )
...
class TestDatabasePerformance:
    ...
    def test_query_performance_with_indices(self, db_session: Session):
        ...
        user_by_username = (
            db_session.query(User).filter(User.username == "perftest5").first()
        )
        ...
        start_time = time.time()
        user_by_email = (
            db_session.query(User).filter(User.email == "perf5@test.com").first()
        )
    ...
    def test_bulk_operations(self, db_session: Session):
        ...
        recipe_count = (
            db_session.query(Recipe).filter(Recipe.title.like("Bulk Recipe%")).count()
        )
        ...
        start_time = time.time()
        bulk_recipes = (
            db_session.query(Recipe).filter(Recipe.title.like("Bulk Recipe%")).all()
        )
