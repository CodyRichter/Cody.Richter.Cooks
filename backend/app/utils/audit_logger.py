...
class SecurityAuditLogger:
    ...
    def log_suspicious_activity(
        ...
        """
        details = additional_details or {}
        details.update({"activity_type": activity_type, "description": description})
    ...
    def get_user_security_events(
        ...
        if event_types:
            query = query.filter(
                SecurityAuditLog.event_type.in_([et.value for et in event_types])
            )

        if risk_levels:
            query = query.filter(
                SecurityAuditLog.risk_level.in_([rl.value for rl in risk_levels])
            )
    ...
    def get_high_risk_events(
        ...
        return (
            ...
            .filter(
                ...
                SecurityAuditLog.risk_level.in_(
                    [SecurityRiskLevel.HIGH.value, SecurityRiskLevel.CRITICAL.value]
                ),
            )
    ...
    def analyze_failed_login_attempts(
        ...
        if username:
            ...
            query = query.filter(
                SecurityAuditLog.details.contains(f'"username": "{username}"')
            )
            ...
            if attempt.details:
                try:
                    ...
                    if "username" in details:
                        username_counts[details["username"]] = (
                            username_counts.get(details["username"], 0) + 1
                        )
